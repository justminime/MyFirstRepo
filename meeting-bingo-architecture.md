...
```mermaid
flowchart LR
    subgraph CG[Card Generation Flow]
        A1[Select Category] --> B1[Shuffle Words] --> C1[Pick 24 Words] --> D1[Render Card]
    end

    subgraph SR2[Speech Recognition Flow]
        A2[Audio Input] --> B2[Speech API] --> C2[Transcript Text] --> D2[Word Detection]
        D2 --> E2{Match Found}
        E2 --> F2[Fill Square] --> G2[Update UI]
    end

    subgraph WD[Win Detection Flow]
        A3[Square Filled] --> B3[Check Lines] --> C3{BINGO} --> D3[Celebrate and Share]
    end
```
...
graph TB
    ...
    MP --> C[Option C: Firebase Realtime Database]
```
